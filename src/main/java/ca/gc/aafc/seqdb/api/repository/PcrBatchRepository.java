...
import ca.gc.aafc.seqdb.api.mapper.PcrBatchMapper;
...
public class PcrBatchRepository extends DinaRepositoryV2<PcrBatchDto, PcrBatch> {
  ...
  public PcrBatchRepository(
    ...
    super(
      ...
      Optional.empty(),
      PcrBatchMapper.INSTANCE,
      PcrBatchDto.class,
      PcrBatch.class,
      props, objMapper, new DinaMappingRegistry(PcrBatchDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }

  @Override
  protected Link generateLinkToResource(PcrBatchDto dto) {
    try {
      return linkTo(methodOn(PcrBatchRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @PostMapping(path = PcrBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(PcrBatchDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
      throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(PcrBatchDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = PcrBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(PcrBatchDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(PcrBatchDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = PcrBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = PcrBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(PcrBatchDto.TYPENAME + "/{id}")
  @Transactional
