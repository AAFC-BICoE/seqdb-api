...
import ca.gc.aafc.seqdb.api.mapper.SeqSubmissionMapper;
...
public class SeqSubmissionRepository extends DinaRepositoryV2<SeqSubmissionDto, SeqSubmission> {
  ...
  public SeqSubmissionRepository(
    ...
    super(
      ...
      Optional.empty(),
      SeqSubmissionMapper.INSTANCE,
      SeqSubmissionDto.class, SeqSubmission.class,
      props, objMapper, new DinaMappingRegistry(SeqSubmissionDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }

  @Override
  protected Link generateLinkToResource(SeqSubmissionDto dto) {
    try {
      return linkTo(methodOn(SeqSubmissionRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @PostMapping(path = SeqSubmissionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(SeqSubmissionDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
      throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(SeqSubmissionDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = SeqSubmissionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(SeqSubmissionDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(SeqSubmissionDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = SeqSubmissionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = SeqSubmissionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(SeqSubmissionDto.TYPENAME + "/{id}")
  @Transactional
