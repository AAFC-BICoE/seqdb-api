...
import ca.gc.aafc.seqdb.api.mapper.MetagenomicsBatchMapper;
...
public class MetagenomicsBatchRepository extends DinaRepositoryV2<MetagenomicsBatchDto, MetagenomicsBatch> {
  ...
  public MetagenomicsBatchRepository(
    ...
    super(
      ...
      Optional.empty(),
      MetagenomicsBatchMapper.INSTANCE,
      MetagenomicsBatchDto.class,
      MetagenomicsBatch.class,
      props, objMapper, new DinaMappingRegistry(MetagenomicsBatchDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }

  @Override
  protected Link generateLinkToResource(MetagenomicsBatchDto dto) {
    try {
  ...
  @PostMapping(path = MetagenomicsBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(MetagenomicsBatchDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
      throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(MetagenomicsBatchDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = MetagenomicsBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(MetagenomicsBatchDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(MetagenomicsBatchDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = MetagenomicsBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = MetagenomicsBatchDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(MetagenomicsBatchDto.TYPENAME + "/{id}")
  @Transactional
