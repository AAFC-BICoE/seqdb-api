...
import ca.gc.aafc.seqdb.api.mapper.PcrBatchItemMapper;
...
public class PcrBatchItemRepository extends DinaRepositoryV2<PcrBatchItemDto, PcrBatchItem> {
  ...
  @Override
  protected Link generateLinkToResource(PcrBatchItemDto dto) {
    try {
      return linkTo(methodOn(PcrBatchItemRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @PostMapping(path = PcrBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(PcrBatchItemDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
    throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(PcrBatchItemDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = PcrBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(PcrBatchItemDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(PcrBatchItemDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = PcrBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = PcrBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(PcrBatchItemDto.TYPENAME + "/{id}")
  @Transactional
