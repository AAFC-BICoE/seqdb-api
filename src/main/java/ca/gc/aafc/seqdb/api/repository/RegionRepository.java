...
import ca.gc.aafc.seqdb.api.mapper.RegionMapper;
...
public class RegionRepository extends DinaRepositoryV2<RegionDto, Region> {
  ...
  @Override
  protected Link generateLinkToResource(RegionDto dto) {
    try {
  ...
  @PostMapping(path = RegionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(RegionDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = RegionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(RegionDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(RegionDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @DeleteMapping(path = RegionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(RegionDto.TYPENAME + "/{id}")
  @Transactional
