...
import ca.gc.aafc.seqdb.api.mapper.MetagenomicsBatchItemMapper;
...
public class MetagenomicsBatchItemRepository extends DinaRepositoryV2<MetagenomicsBatchItemDto, MetagenomicsBatchItem> {
  ...
  public MetagenomicsBatchItemRepository(
    ...
    super(
      ...
      Optional.empty(),
      MetagenomicsBatchItemMapper.INSTANCE,
      MetagenomicsBatchItemDto.class,
      MetagenomicsBatchItem.class,
      props, objMapper, new DinaMappingRegistry(MetagenomicsBatchItemDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }
  @Override
  protected Link generateLinkToResource(MetagenomicsBatchItemDto dto) {
    try {
  ...
  @PostMapping(path = MetagenomicsBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(MetagenomicsBatchItemDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
      throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(MetagenomicsBatchItemDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = MetagenomicsBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(MetagenomicsBatchItemDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(MetagenomicsBatchItemDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = MetagenomicsBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = MetagenomicsBatchItemDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(MetagenomicsBatchItemDto.TYPENAME + "/{id}")
  @Transactional
