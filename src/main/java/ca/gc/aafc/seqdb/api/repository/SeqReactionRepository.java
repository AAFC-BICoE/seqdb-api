...
import ca.gc.aafc.seqdb.api.mapper.SeqReactionMapper;
...
public class SeqReactionRepository extends DinaRepositoryV2<SeqReactionDto, SeqReaction> {
  ...
  public SeqReactionRepository(
    ...
    super(
      ...
      Optional.empty(),
      SeqReactionMapper.INSTANCE,
      SeqReactionDto.class, SeqReaction.class,
      props, objMapper, new DinaMappingRegistry(SeqReactionDto.class, true));
    this.dinaAuthenticatedUser = dinaAuthenticatedUser.orElse(null);
  }

  @Override
  protected Link generateLinkToResource(SeqReactionDto dto) {
    try {
      return linkTo(methodOn(SeqReactionRepository.class).onFindOne(dto.getUuid(), null)).withSelfRel();
  ...
  @PostMapping(path = SeqReactionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(SeqReactionDto.TYPENAME + "/{id}")
  public ResponseEntity<RepresentationModel<?>> onFindOne(@PathVariable UUID id, HttpServletRequest req)
    throws ResourceNotFoundException, ResourceGoneException {
    return handleFindOne(id, req);
  }

  @GetMapping(SeqReactionDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = SeqReactionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(SeqReactionDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(SeqReactionDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @PatchMapping(path = SeqReactionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(path = SeqReactionDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(SeqReactionDto.TYPENAME + "/{id}")
  @Transactional
