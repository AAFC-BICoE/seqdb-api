...
import ca.gc.aafc.seqdb.api.mapper.IndexSetMapper;
...
public class IndexSetRepository extends DinaRepositoryV2<IndexSetDto, IndexSet> {
  ...
  @Override
  protected Link generateLinkToResource(IndexSetDto dto) {
    try {
  ...
  @PostMapping(path = IndexSetDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_LOAD_PATH, consumes = JSON_API_BULK)
  ...
  @GetMapping(IndexSetDto.TYPENAME)
  public ResponseEntity<RepresentationModel<?>> onFindAll(HttpServletRequest req) {
    return handleFindAll(req);
  }

  @PostMapping(path = IndexSetDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @PostMapping(IndexSetDto.TYPENAME)
  @Transactional
  ...
  @PatchMapping(IndexSetDto.TYPENAME + "/{id}")
  @Transactional
  ...
  @DeleteMapping(path = IndexSetDto.TYPENAME + "/" + DinaRepositoryV2.JSON_API_BULK_PATH, consumes = JSON_API_BULK)
  @Transactional
  ...
  @DeleteMapping(IndexSetDto.TYPENAME + "/{id}")
  @Transactional
